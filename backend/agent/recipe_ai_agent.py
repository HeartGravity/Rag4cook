...
class RecipeAgent:
    ...
    def __init__(self, api_key: str, base_url: str = "https://dashscope.aliyuncs.com/compatible-mode/v1", model_name: str = "qwen3.6-flash"):
        ...
        # 排除的目录
        self.excluded_directories = ["template", ".github", ".git", "starsystem"]
...
6. **标签 (tags)**：生成3-4个有助于检索此文档的关键词，关键词需要体现出文档内容的关键信息。
...
class RecipeKnowledgeGraphBuilder:
    ...
    def _init_relationship_mappings(self):
        ...
        self.relationship_type_mapping = {
            ...
            "prep_time": "801000011",
            "related_to_tool": "801000012",
            "related_to_ingredient": "801000013",
            "related_to_method": "801000014"
        }
    ...
    def process_tip(self, markdown_content: str, file_path: str) -> Dict:
        """处理单个厨房技巧"""
        tip_info = self.ai_agent.extract_tip_info(markdown_content, file_path)
        tip_id = self.generate_concept_id()
        
        # 将列表转换为竖线分隔的字符串以便保存到 Neo4j CSV
        warnings_str = "|".join(tip_info.safety_warnings) if tip_info.safety_warnings else ""
        tags_str = ",".join(tip_info.tags) if tip_info.tags else ""
        
        tip_concept = {
            "concept_id": tip_id,
            "concept_type": "KitchenTip", # 新的节点标签
            "name": tip_info.name,
            "preferred_term": tip_info.name,
            "category": tip_info.category,
            "description": tip_info.description,
            "safety_warnings": warnings_str,
            "tags": tags_str,
            "file_path": file_path
        }
        self.concepts.append(tip_concept)
        
        # 辅助函数：将关联的实体独立创建节点并建立关系
        def link_related_entity(entities: List[str], entity_type: str, rel_type: str):
            for entity_name in entities:
                ent_id = self.generate_concept_id()
                self.concepts.append({
                    "concept_id": ent_id,
                    "concept_type": entity_type,
                    "name": entity_name,
                    "preferred_term": entity_name
                })
                self.relationships.append({
                    "relationship_id": f"R_{len(self.relationships) + 1:06d}",
                    "source_id": tip_id,
                    "target_id": ent_id,
                    "relationship_type": self.relationship_type_mapping[rel_type]
                })

        link_related_entity(tip_info.related_tools, "CookingTool", "related_to_tool")
        link_related_entity(tip_info.related_ingredients, "Ingredient", "related_to_ingredient")
        link_related_entity(tip_info.related_methods, "CookingMethod", "related_to_method")
    ...
    def batch_process_recipes(self, recipe_dir: str, resume: bool = True) -> Tuple[int, int]:
        ...
        target_files = []
        for folder in ["dishes", "tips"]:
            folder_path = os.path.join(recipe_dir, folder)
            if os.path.exists(folder_path):
                target_files.extend(glob.glob(os.path.join(folder_path, "**/*.md"), recursive=True))

        if not target_files:
            target_files = glob.glob(os.path.join(recipe_dir, "**/*.md"), recursive=True)
        ...
        print(f"共扫描到待处理的 Markdown 文件: {len(target_files)} 个")
        recipe_files = target_files
        ...
        try:
            for i, recipe_file in enumerate(remaining_files):
                try:
                    ...
                    relative_path = os.path.relpath(recipe_file, recipe_dir)
                    path_parts = relative_path.replace('\\', '/').split('/')
                    if "tips" in path_parts:
                        # 处理厨房技巧
                        self.process_tip(content, relative_path)
                    else:
                        # 处理菜谱
    ...
    def export_to_neo4j_csv(self, output_dir: str, merge_batches: bool = True):
        ...
        for concept in final_concepts:
            ...
            elif concept["concept_type"] == "KitchenTip":
                node.update({
                    "description": concept.get("description", ""),
                    "safetyWarnings": concept.get("safety_warnings", ""),
                    "tags": concept.get("tags", ""),
                    "filePath": concept.get("file_path", "")
                })
...
SET n.nodeId = row.nodeId,
    ...
    n.tools = row.tools,
    n.safetyWarnings = row.safetyWarnings,
...
def main():
    ...
    parser.add_argument('-k', '--api-key', required=True, help='API密钥')
    ...
    # 创建AI agent
    print("初始化AI Agent...")
    ai_agent = RecipeAgent(args.api_key, args.base_url)
